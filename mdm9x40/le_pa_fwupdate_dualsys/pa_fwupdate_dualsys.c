...
)
{
    ...
    if (hdrPtr->imageType == CWE_IMAGE_TYPE_SBL1)
    {
        ...
        return WriteDataSBL( format, hdrPtr, lengthPtr, offset, dataPtr, forceClose, isFlashed );
    }
        ...
        if (result == LE_OK)
        {
            ...
            if (bSync == true)
            {
                ...
                else
                {
                    ...
                    if (-1 == fcntl(fd, F_SETFL, (int)flags))
                    {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        if (readCount > 0)
                        {
                            ...
                            {
                                lenRead = read (fd, bufferPtr + readCount,
                                                DataLenToBeRead - readCount);
                                if ((!lenRead) || ((-1 == lenRead) && (EAGAIN == errno)))
                                {
                                    ...
                                    else if ((1 == rc) && FD_ISSET(fd, &fdSetRead))
                                    {
                                        lenRead = read (fd, bufferPtr + readCount,
                                                        DataLenToBeRead - readCount);
                                        // If read(2) returns 0 as length read, this is an error
                                        // because a length > 0 is expected here
                                        if (!lenRead)
                                        {
...
error:
    ...
    if (LE_OK != WriteData(CurrentImageFormat, &CurrentCweHeader, NULL, 0, NULL, true, NULL))
    {
